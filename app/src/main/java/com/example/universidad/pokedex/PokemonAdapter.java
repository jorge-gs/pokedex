...
public class PokemonAdapter extends RecyclerView.Adapter<PokemonAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(ViewHolder holder, int position) {
        holder.networkImage.setImageUrl(Pokemon.pokemonList.get(position).imageURL, VolleySingleton.getInstance(holder.itemView.getContext()).getImageLoader());
        holder.description.setText(Pokemon.pokemonList.get(position).entry + ". " + Pokemon.pokemonList.get(position).name);
    ...
    public int getItemCount() {
        return Pokemon.pokemonList.size();
    }
}
